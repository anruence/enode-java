...
public class SendKafkaService implements IMQProducer {
    ...
    private ProducerRecord<String, String> buildCommandMessage(ICommand command, String routeKey, boolean needReply) {
        ...
        //命令唯一id，聚合根id
        String key = String.format("%s%s", command.id(), command.getAggregateRootId() == null ? "" : MessageConst.KEY_SEPARATOR + "cmd_agg_" + command.getAggregateRootId());
        ProducerRecord message = new ProducerRecord<>(
                topicTagData.getTopic(), getPartionByRouteKey(routeKey, topicTagData.getTopic()), key, messageData);
        return message;
    }
