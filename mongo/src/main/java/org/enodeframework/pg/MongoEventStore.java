...
public class MongoEventStore implements IEventStore {
    ...
    private CompletableFuture<AggregateEventAppendResult> batchAppendAggregateEventsAsync(String aggregateRootId, List<DomainEventStream> eventStreamList) {
        ...
        return future.exceptionally(throwable -> {
            if (throwable instanceof MongoWriteException) {
                ...
                if (ex.getCode() == duplicateCode && ex.getMessage().contains(versionIndexName)) {
                ...
                if (ex.getCode() == duplicateCode && ex.getMessage().contains(commandIndexName)) {
                    ...
                    eventStreamList.forEach(eventStream -> {
                        Task.await(tryFindEventByCommandIdAsync(aggregateRootId, eventStream.getCommandId(), appendResult.getDuplicateCommandIds(), 0));
                    });
                    return appendResult;
                }
    ...
    public static String parseCommandIdInException(String errMsg) {
